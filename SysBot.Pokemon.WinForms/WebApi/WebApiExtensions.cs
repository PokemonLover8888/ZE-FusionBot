...
public static class WebApiExtensions
{
    ...
    private static void CleanupStalePortFiles()
    {
        try
        {
            ...
            var portFiles = Directory.GetFiles(exeDir, "PKM-Universe-Bot_*.port");
            ...
            foreach (var portFile in portFiles)
            {
                try
                {
                    ...
                    var pidStr = fileName.Substring("PKM-Universe-Bot_".Length);
                    ...
                    if (int.TryParse(pidStr, out int pid))
                    {
                        ...
                        try
                        {
                            ...
                            if (process.ProcessName.Contains("SysBot", StringComparison.OrdinalIgnoreCase) ||
                                process.ProcessName.Contains("PKM-Universe-Bot", StringComparison.OrdinalIgnoreCase))
                            {
                                continue;
                            }
                        }
    ...
    private static void CreatePortFile()
    {
        try
        {
            var exePath = System.Diagnostics.Process.GetCurrentProcess().MainModule?.FileName ?? "";
            var exeDir = Path.GetDirectoryName(exePath) ?? Program.WorkingDirectory;
            var portFile = Path.Combine(exeDir, $"PKM-Universe-Bot_{Environment.ProcessId}.port");
        ...
        try
        {
            ...
            foreach (var process in processes)
            {
                try
                {
                    ...
                    var portFile = Path.Combine(Path.GetDirectoryName(exePath)!, $"PKM-Universe-Bot_{process.Id}.port");
                    if (!File.Exists(portFile))
                        continue;
